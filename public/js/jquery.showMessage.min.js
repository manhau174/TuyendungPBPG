...
jQuery(function(){jQuery(document).keydown(function(a){if((a===null?event.keyCode:a.which)==27){jQuery("#showMessage").fadeOut();typeof t!="undefined"&&clearTimeout(t)}});jQuery.showMessage=function(a){a=jQuery.extend({thisMessage:[""],className:"notification",position:"top",opacity:90,displayNavigation:true,autoClose:false,delayTime:5E3},a);jQuery("#showMessage").length&&jQuery("#showMessage").remove();var b=jQuery("<div></div>").css({display:"none",position:"fixed","z-index":9999,left:0,width:"100%",
margin:0,filter:"Alpha(Opacity="+a.opacity+")",opacity:a.opacity/100}).attr("id","showMessage").addClass(a.className);a.position=="bottom"?jQuery(b).css("bottom",0):jQuery(b).css("top",0);if(a.displayNavigation){var c=jQuery("<span></span>").css({"float":"right","padding":"5px 1em 0px 0px","font-weight":"bold","font-size":"small"}).html("Esc Key or "),d=jQuery("<a></a>").attr({href:"",title:"close"}).css("text-decoration","underline").click(function(){jQuery("#showMessage").fadeOut();clearTimeout(t);return false}).text("close");
...
jQuery("<li></li>").html(a.thisMessage[e]);jQuery(d).append(f)}jQuery(c).append(d).append('<div style="clear:both"></div>');jQuery(b).append(c);jQuery("body").append(b);jQuery(b).fadeIn();a.autoClose&&closeMessage(a.delayTime)}});